...
- **Extension registry**: runtime-added MCP servers and exec skills managed via Telegram (`add_extension`, `remove_extension`, `list_extensions`), persisted to `extensions.json`, loaded at startup, works in both direct API and CLI modes, env var filtering (LD_PRELOAD/DYLD_* blocklist) for MCP extensions in CLI config
