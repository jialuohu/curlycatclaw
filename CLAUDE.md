...
- **Observation memory**: automatic extraction of decisions, preferences, and project state from conversations; `ObservationExtractor` in `internal/memory/observer.go` runs after idle detection (in-memory turn counter); SQLite tables (`observations`, `observation_facts`, `observation_extraction_state`) store structured observations; Qdrant collection `curlycatclaw_observations` for semantic search; system prompt "What I remember" section injects relevant observations with dedup against facts; parallel Qdrant queries for observations and summaries; opt-in via `[memory.observations]` config
...
| `skills/observation.go` | Observation skills (search, list, get, forget) |
