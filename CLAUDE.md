...
**Claude integration**: Two modes via `[claude]` config. Direct API (`api_key`) uses anthropic-sdk-go with streaming. CLI subprocess (`cli_path` + `oauth_token`) spawns long-lived `claude` processes per user. `thinking_effort` controls extended thinking (high=10K, xhigh=20K, max=32K budget tokens; `xhigh` requires Opus 4.7+). `/effort`, `/retry`, `/stop`, `/debug` Telegram commands for runtime control. `/update`, `/status`, `/rollback` for self-update management. Document attachments: PDFs sent as native document blocks (both modes), text files inlined into message (CLI) or as text blocks (API).
...
- MCP subprocess debug log: `/data/mcp-debug.log` captures startup extension loading AND runtime hot-reload events (connect attempts, successes, failures). MCP subprocess stderr goes to Claude CLI (invisible in `docker compose logs`), so this file is the only way to debug MCP issues in CLI mode. Hot-reload entries appear live as `add_extension` fires because `mcpHotReloader` carries a `dbgLog` that the caller wires in after the file is opened. "Startup" here is per-CLI-spawn, NOT per-container-start: the log stays empty after a `docker compose up -d` until a user message (via Session Actor → Claude CLI) or cron fire (via `SpawnOneShot`) triggers a fresh Claude CLI child, which spawns its own MCP subprocess. To verify an MCP code change took effect, trigger an agent interaction or wait for the next cron — don't expect new lines on restart alone.
...
- `CURLYCATCLAW_MASTER_KEY` plumbing: lives in `<repo>/.env` next to `docker-compose.yml` (gitignored), NOT in `~/.curlycatclaw/env` (legacy location, unread since Apr 7 commit `0b146f6`). `docker-compose.yml` passes it through via `${CURLYCATCLAW_MASTER_KEY:-}` alongside `UPDATER_SECRET`. From the container it reaches the MCP subprocess via a per-PID tmpfile (`/tmp/curlycatclaw-mk-<pid>`) written by `buildMCPConfig` at `internal/session/actor.go:2465`; the subprocess reads it via `CURLYCATCLAW_MASTER_KEY_FILE`. When the key is missing AND `credentials.enc` exists, both `main.go` and `mcp_server.go:initCredStore` log a startup WARN pointing at `.env`. Silent credStore init means `set_extension_env`/`unset_extension_env` don't register and existing encrypted refs don't resolve — always check the WARN first when those tools go missing.
- Debugging SQLite state: neither the container image nor the host typically has `sqlite3` installed. For ad-hoc queries against `~/.curlycatclaw/curlycatclaw.db`, use a throwaway sidecar: `docker run --rm -v ~/.curlycatclaw:/data keinos/sqlite3 sqlite3 /data/curlycatclaw.db "<query>"`. Handy tables: `reminders` (cron + one-time, with `cron_expr`, `fire_at`, `status`, `prompt`), `messages` (role/content/created_at), `conversations`, `tool_calls`, `observations`, and the `*_fts` shadow tables. Use `".schema <table>"` to check column names before writing `SELECT` queries — schema has migrated in-place several times.
- gocron scheduler timezone: `ReminderActor.Run` must create the scheduler via `newCronScheduler(ra.lastLoc)` (set from `ra.locFn()` at Run start) in `skills/remind.go`, passing `gocron.WithLocation(...)` so cron expressions evaluate in the user's configured timezone, not the container's `time.Local` (usually UTC). A bare `gocron.NewScheduler()` makes `0 6 * * *` fire at 06:00 UTC = 23:00 PDT the previous day for a PDT-configured user — 7 hours early. One-time reminders are unaffected (they carry their own zone via absolute `time.Time`). The same invariant applies to the rebuild path in `maybeRebuildScheduler` (see the "Timezone hot-swap" gotcha below). Regression guard: `skills/remind_test.go:TestNewCronScheduler_FiresInConfiguredLocation` schedules the same cron in two zones and asserts the next-runs produce distinct UTC instants, which catches the regression regardless of host `time.Local`.
- Cron CLI spawn MCP config parity: `CronExecutor.buildSpawnParams` uses the package-level `buildMCPConfigForUser(cfg, configPath, userID, chatID)` helper (extracted from `Actor.buildMCPConfig` in `internal/session/actor.go`) so cron-fired CLI subprocesses see the same MCP server list as the interactive session — including `curlycatclaw-skills` and every runtime extension proxied through it (paper-search-mcp, scrapling, etc.). A bare `SpawnParams{}` with no `MCPConfig` spawns the CLI with zero MCP servers; cron tools then report "not registered" and fall back to built-in `WebSearch`/`WebFetch`. Added cost: ~2-3s MCP init + parallel upstream loads (capped at 15s each) on every cron fire. When wiring a new CLI spawn callsite (summarizer, observer, eval), decide whether it needs runtime MCP — pure-prompt tasks leave `MCPConfig` empty to skip the init.
- Cron prompts referencing MCP tools must either use the FULL namespaced name (`mcp__curlycatclaw-skills__paper-search-mcp__search_papers`) OR rely on the agent to match bare-name suffixes via the hint wired into `CronExecutor.buildSystemPrompt`. Tools exposed through the `curlycatclaw-skills` proxy get double-namespaced: the proxy prefixes `<upstream>__<tool>` (see `mcpProxySep = "__"` in `cmd/curlycatclaw/mcp_server.go:registerProxyTool`), then Claude CLI prefixes again with `mcp__<config-server>__`, producing the full 4-segment name. The cron's minimal system prompt doesn't teach the agent about Claude's `mcp__` prefix, so a prompt that says "use `search_papers`" literally can make a fresh cron agent say "tools not loaded, falling back to WebSearch" even when the MCP subprocess logs `OK proxying paper-search-mcp tools=60`. The Apr 17 fix in `cron.go:buildSystemPrompt` adds a namespacing hint + explicit "do NOT declare a tool missing without searching for its suffix" guard. Regression: `TestBuildSystemPrompt_IncludesMCPNamingHint`. When debugging "agent can't see tool X": first probe the MCP subprocess directly (`echo '...initialize...tools/list...' | curlycatclaw --mcp-server`) to prove it's exposed, then probe Claude CLI with `--mcp-config` to see the Claude-side name, then inspect the prompt wording.
- Claude CLI MCP probe (companion to the MCP subprocess probe above): write `/tmp/cfg.json` inside the container with `{"mcpServers":{"curlycatclaw-skills":{"command":"/usr/local/bin/curlycatclaw","args":["--mcp-server"],"env":{"CURLYCATCLAW_USER_ID":"<uid>","CURLYCATCLAW_CHAT_ID":"<cid>","CURLYCATCLAW_DB_PATH":"/data/curlycatclaw.db","CURLYCATCLAW_CONFIG":"/data/config.toml","CURLYCATCLAW_MASTER_KEY":"<key>"}}}}` (env shape matches `buildMCPConfigForUser`; include `CURLYCATCLAW_MASTER_KEY` only when exercising credential-store tools). Then `docker compose exec -T -e CLAUDE_CODE_OAUTH_TOKEN=<from config.toml oauth_token> curlycatclaw claude --print --strict-mcp-config --mcp-config /tmp/cfg.json --dangerously-skip-permissions --model claude-opus-4-7 <<< "list tools matching X"`. The oauth token lives in `~/.curlycatclaw/config.toml` under `[claude].oauth_token` (NOT `.env` — that file only carries `CURLYCATCLAW_MASTER_KEY` and `UPDATER_SECRET`). Returns the actual 4-segment names Claude sees (`mcp__curlycatclaw-skills__<upstream>__<tool>`), which is often the missing piece when a prompt references a bare name and the agent reports "tool not found."
- Cron-fired turns persist to conversation history via `ReminderActor.SetConversationPersister(*memory.Store)` (wired in `main.go`). After a successful cron fire (or error), `persistCronTurn` writes a `user`-role marker (`[Cron reminder fired: <title>] Scheduled: <time> Prompt: <prompt>`) and the `assistant`-role text that went to Telegram. Without this, the interactive agent has no record of its own cron sends and fabricates answers when asked about them. Nil-safe setter — unit tests without a persister exercise the pre-v0.36.8 fire-and-forget path. Persistence failures log warnings but never block the Telegram send.
- Reminder lifecycle is two-step: `cancel_reminder` soft-archives (status=`cancelled`, leaves the row), `delete_reminder` purges cancelled/fired rows from the table. `delete_reminder` refuses pending rows with an error naming `cancel_reminder` so the agent's autorepair loop does cancel-then-delete. `list_reminders` defaults to `status="pending"` — pass `status="all"` to include tombstones. Changing the wording of the "call cancel_reminder first" error will break the autorepair cue — `skills/remind_test.go:TestDeleteReminder_RefusesPending` asserts the exact phrase. Cross-user deletes return "not found" to avoid leaking existence (IDOR-safe).
- Per-reminder `effort` override: `set_reminder`/`update_reminder` accept an optional `effort` field (low/medium/high/xhigh/max) that overrides the config default for this one cron task's Claude run. Validated via `validateReminderEffort` → `config.ValidEffort` (single source of truth for the enum). Stored in the `effort TEXT` column on `reminders`, rendered as `[effort: xhigh]` in list_reminders/get_reminder output. At fire time, `ReminderActor.fireCronTask` reads `r.Effort` and passes it through `CronRunner.Execute(..., effort, ...)` to `CronExecutor.Execute` → `SpawnParams.Effort` → Claude CLI's `--effort` flag. **Both CLI and API modes plumb effort.** `Execute` resolves `effectiveEffort = per-reminder || config-default` then dispatches — CLI flows through `SpawnParams.Effort` → `--effort` flag; API flows through `runToolLoop(..., effort)` → `SendParams.ThinkingEffort`. Before the Apr 17 fix, `runToolLoop` constructed `SendParams` without `ThinkingEffort` and silently dropped ALL effort in API mode (pre-existing latent bug hidden until the new `[effort: X]` display made it user-visible). Startup log line `cron: executing` now emits `effort_source=per-reminder|config-default` so "why isn't my reminder thinking?" debugging surfaces both the resolved value and its origin. `CronRunner` interface (in `skills/remind.go`, to avoid the skills ↔ session cycle) now takes `(userID, chatID, prompt, model, effort string, scheduledAt)` — changing it still requires in-sync updates to the interface, `session.CronExecutor.Execute`, and `skills.mockCronRunner` (plus every call site of `ce.Execute` in `cron_test.go`). Regression guards: `TestSetReminder_AcceptsValidEffort`, `TestSetReminder_RejectsInvalidEffort`, `TestUpdateReminder_EffortField`, `TestFireCronTask_PassesEffortToRunner`.
- Adding a new thinking-effort level (after `low/medium/high/xhigh/max`) touches 7+ places with no compile-time link — `config.Effort` is the enum source but display strings live as hardcoded slices/regexes all over. Update in order of visibility: (1) `config/config.go` — new `Effort` const + add to `ValidEffort` switch + update the `claude.thinking_effort must be one of...` error string in `validate()`; (2) `internal/claude/client.go:effortBudget` — add budget_tokens preset if it's an extended-thinking level (direct API mode); (3) `internal/session/actor.go` — THREE user-visible strings in `handleEffortCommand` (help text + "Unknown effort level" error) plus `handleRetryCommand`'s "Valid: ..." error; (4) `internal/telegram/channel.go:~218` — slash-command description (Telegram autocomplete); (5) `skills/remind.go` — `set_reminder` AND `update_reminder` InputSchema `effort` enum array (`validateReminderEffort` already delegates to `config.ValidEffort` so no change there); (6) `config.toml.example`, `docs/configuration.md`, `docs/architecture.md`, `README.md`, and this file's mention of effort levels; (7) `config/config_test.go` — BOTH `TestValidEffort` AND `TestValidate_ValidThinkingEffort` iterate hardcoded slices. Tests cover (1)(2) well; the display-string drift (3)(4)(6) is where adversarial review earns its keep. Apr 17 `xhigh` rollout missed 5 of these until a second-pass review caught them.
- In-place reminder edits go through `update_reminder` (partial update, pending-only, user-scoped). In CLI mode the signal channel passed to `InitRemindSkills` drains to `/dev/null` (`cmd/curlycatclaw/mcp_server.go` — same pattern documented for `cancel_reminder`), so the skill can NOT rely on signal delivery. The actor has three layers that together make updates reach the scheduler: (1) `fireReminder` re-reads the full row from DB before every fire so `message`/`prompt`/`model` edits show up at the next fire even without a reschedule; (2) `pollNewReminders` Phase 1.5 compares each tracked pending row's DB `fire_at`/`cron_expr` against a per-id `jobMeta` snapshot taken at schedule time and reschedules on divergence (10s worst-case latency in CLI mode); (3) `handleSignal` also cancels any existing tracked job before rescheduling so API-mode signals don't leave a stale gocron job firing at the old time next to the new one. All three must stay in sync — `scheduleReminder` writes `jobMeta`, `cancelJob` and the one-time task closure clear it. `fireReminder` preserves the originally-scheduled `FireAt` in the fresh row so cron-task output reports the real scheduled time (not a post-hoc rewrite). `update_reminder` rejects `fire_at` on recurring reminders because `fire_at` is surfaced to Claude as `scheduledAt` in the cron system prompt, so letting it be rewritten would silently corrupt that record without actually changing when the job fires. All three schedule sites (loadPendingReminders, handleSignal, pollNewReminders Phases 1.5+2) go through a shared `scheduleOrFire` helper so a past-due `fire_at` on a one-time reminder fires immediately instead of silently no-oping inside gocron — symmetric with set_reminder's startup-recovery behavior. Non-pending rows return an error naming `set_reminder` as the recreate path. Empty-string inputs mean "don't change" (matches `update_observation` precedent); clearing an optional field requires cancel + recreate. Regression guards: `TestUpdateReminder_RefusesCancelled` (autorepair phrase), `TestUpdateReminder_RejectsFireAtOnRecurring` (timestamp-corruption guard), `TestReminderActor_PollDetectsScheduleChange` (CLI-mode poll rescheduling), `TestFireReminder_ReReadsMessageFromDB` + `TestFireCronTask_ReReadsContentFromDB` (stale-closure content fix for plain + cron variants), `TestReminderActor_UpdateReschedulesJob` (API-mode signal path), `TestReminderActor_UpdateToPastFireAt` (past-due update fires immediately).
- `telegram.OutgoingMessage{HTML: true}` is the OPT-IN for markdown→HTML conversion via `mdhtml.ConvertSafe` + `ParseMode=HTML`. Any send that carries Claude-generated text (or any markdown) MUST set it; otherwise `**`/`###`/`[](url)` reach Telegram as raw characters. Interactive session sets it everywhere (`internal/session/actor.go`); `ReminderActor.fireCronTask` previously didn't and shipped raw markdown for as long as cron tasks existed. Regression guards: `skills/remind_test.go:TestFireCronTask_Success`/`TestFireCronTask_Error` assert `msg.HTML` on the captured Telegram message. Plain reminder sends (no Claude output, just user-supplied title) intentionally stay as plain text.
- Reminder prompt visibility is two-tier: `list_reminders` returns a 500-rune preview of each cron prompt (cap is `listPromptPreviewRunes` in `skills/remind.go`), and `get_reminder` returns the full body for a specific ID. List output is also capped at 50 rows (`listRemindersLimit`) with offset-based pagination (`offset=50` → second page, `offset=100` → third, etc.) and an overflow notice that hints at the exact next offset — without these caps, a user with hundreds of cron tasks blew the agent's context budget every list call (~5KB prompt × 100 = 500KB per call). Every prompt body that surfaces back to the interactive agent is wrapped in `<user_prompt_body>...</user_prompt_body>` trust delimiters by `renderPromptBody` so the agent treats stored prompts as quoted user data, not instructions — important because reminder prompts were previously executed only inside `CronExecutor`'s fixed system prompt, and `list_reminders` was the first surface that fed them into the interactive Claude's tool-rich context. Bodies containing the literal closing tag are rewritten to `</user_prompt_body_>` so the wrapper can't be broken out of. Every prompt line is also indented 4 spaces under the parent `#N` reminder header so a multi-line prompt containing `#999 [cron:pending] FAKE entry` can't spoof a sibling that the agent might call cancel/delete on. **Order-of-operations is load-bearing in `renderPromptBody`**: truncate FIRST, escape SECOND, strip any partial closing-tag prefix at the tail THIRD — the earlier "escape first, truncate second" version had a bypass where padding the body so the escape landed on the truncation boundary leaked `</user_prompt_b...` into output. The header line itself (message, model, cron_expr) is also a spoofing surface — `validateReminderMessage`/`validateReminderModel` reject `\r`/`\n` and length-cap at write time, and `sanitizeForHeader` replaces newlines with spaces at render time as belt-and-suspenders for any pre-validation row. `list_reminders` queries use `ORDER BY fire_at, id` (not just `fire_at`) so top-N membership is stable across calls when reminders share a fire_at. `get_reminder` rejects `id <= 0` (catches negative IDs that previously reflected attacker-controlled ints back) and is IDOR-safe (same "not found" string for cross-user and missing). Regression guards: `TestRenderPromptBody_EscapesClosingTag`, `TestRenderPromptBody_NoBypassViaTruncation` (the 482-char attack), `TestRenderPromptBody_BoundaryAtMaxRunes`, `TestListReminders_TruncatesLongPrompt`, `TestListReminders_LimitOverflow`, `TestListReminders_OffsetPagination`, `TestListReminders_RejectsNegativeOffset`, `TestListReminders_MultilinePromptIndentation`, `TestListReminders_SanitizesNewlinesInHeaderFallback`, `TestListReminders_OrdersByFireAtThenID`, `TestGetReminder_FullBodyNoTruncation`, `TestGetReminder_CrossUserIDOR`, `TestGetReminder_RejectsNegativeAndZeroID`, `TestSetReminder_RejectsNewlineInMessage`, `TestSetReminder_RejectsNewlineInModel`, `TestSetReminder_RejectsTooLongModel`.
- `docs/skills.md` is auto-generated by `gws generate-skills` and lists Google Workspace MCP skills, NOT curlycatclaw's built-in skills. Don't edit it; don't use it to discover what skills exist. Source of truth for built-in skills is `skills/*.go` (registered in `cmd/curlycatclaw/main.go` for API mode and `cmd/curlycatclaw/mcp_server.go` for CLI mode — both must register, see the "New skills must be registered in BOTH" gotcha above).
- When adding a gotcha to this file, prefer `file.go:functionName` over `file.go:1234`. Line numbers drift on the next refactor; symbol names survive renames at worst. Existing entries with bare line refs (e.g., `actor.go:2465`, `skills.go:110`) have already drifted and are kept as historical pointers, but new entries should use symbols so the rot stays bounded.
- Runtime MCP extensions shadow config-declared servers with the same name. `cmd/curlycatclaw/mcp_server.go:loadProxyUpstreams` collects runtime names first, then skips any `cfg.MCP.Servers` entry sharing a name and logs `WARN mcp-server: config MCP server shadowed by runtime extension...` — grep this when debugging "why isn't my config entry loading?" Runtime wins because it holds encrypted env vars via `set_extension_env`; config can't store secrets. `add_extension` preflight-rejects names that exist in config.toml with an error mentioning `config.toml` so the agent's autorepair loop picks a new name or instructs the user to clean up config.toml. `internal/extension/skills.go:addExtensionSkill` takes `configServers []ConfigMCPServer` — both CLI mode (`mcp_server.go`) and API mode (`main.go`) wire it.
- `ReminderActor.mu` is mandatory, not defense-in-depth. Distinct from the session `Actor`'s "no mutex needed" pattern: gocron schedules cron-fire callbacks on its own goroutines, and tests read actor state cross-goroutine to verify state. Fields covered: `jobs`, `jobMeta`, `lastLoc`. Any new `ra.<field>` that gocron callbacks write OR tests read must join the same mutex. The race detector catches violations — see the v0.37.0 `lastLoc` regression caught by `TestReminderActor_TZChangeRebuildsScheduler` (writes happened in `Run()` and `maybeRebuildScheduler`, reads happened in the test goroutine).
- When adding a runtime override for a config field via `system_prefs`, run `grep -rn 'cfg\.<Field>\|cfg\.<Method>()' internal cmd skills` BEFORE designing. Every callsite that captures the value at construction time is a future staleness bug. The v0.37.0 timezone override touched 5 binding sites; 3 in `internal/session/actor.go` were missed on first pass and caught only by plan-eng-review. Two refactor patterns landed for that work and should be reused: (a) `func() *time.Location` parameter instead of fixed `*time.Location` for skill-closure factories that read the value lazily; (b) a single `memory.Effective<X>(cfg, db)` accessor returning `(value, source)` that all consumers share, with `cfg.<Method>()` preserved as the config-only view.
- Timezone hot-swap (`set_timezone` skill): the runtime override lives in `system_prefs[timezone]` (SQLite kv table, migration in `internal/memory/store.go`). Five binding sites read it via `memory.EffectiveLocation(cfg, db)`: (1) `ReminderActor.locFn` (skill-closure factory in `skills/remind.go:InitRemindSkills` — note the `locFn func() *time.Location` signature, called fresh per invocation, NOT captured at init); (2) `ReminderActor.maybeRebuildScheduler` reads it on `tzChangeCh` signal AND on every 10s poll tick (CLI-mode parity since the signal channel drains to /dev/null in the MCP subprocess); (3) `CronExecutor.buildSystemPrompt` for cron-fire system prompt rendering (`internal/session/cron.go:buildSystemPrompt`); (4) interactive session's `Actor.buildSystemPrompt` and the per-message time-injection prefix (3 callsites in `internal/session/actor.go`); (5) the `get_timezone` skill itself. The actor's `lastLoc` field gates rebuilds: same TZ = no-op, different TZ = `scheduler.Shutdown()` (wrapped in 30s timeout to handle a stuck in-flight cron-Claude task) + new `gocron.NewScheduler` + `loadPendingReminders`. **gocron v2's `WithLocation` is fixed at scheduler-creation time**, so `RemoveJob` + re-add into the same scheduler will NOT re-evaluate cron expressions in the new TZ — the full Shutdown-and-rebuild is load-bearing. CLI-mode latency: up to 10s for the poll path to detect a DB-only set_timezone (same SLA as `cancel_reminder`). When adding any new field that captures `cfg.Timezone` or `cfg.Location()` at construction time, audit the existing 5 binding sites first; static capture is the bug class this whole feature exists to fix. Regression guards: `TestEffectiveLocation_OverrideWins`, `TestEffectiveLocation_CorruptedOverrideFallsBack`, `TestEffectiveLocation_NilDBFallsBack`, `TestSetTimezone_PersistsAndSignals`, `TestSetTimezone_TrimsWhitespace`, `TestSetTimezone_NoOpOnSame`, `TestSetReminder_UsesEffectiveTZForFireAtParsing`, `TestReminderActor_TZChangeRebuildsScheduler`, `TestReminderActor_TZChangeNoOpOnSame`.
...
| `skills/remind.go` | Reminder skills + ReminderActor (gocron scheduling, poll-based cancel detection, TZ hot-swap via tzChangeCh) |
| `skills/timezone.go` | set_timezone / get_timezone skills (runtime TZ override, agent-callable) |
| `internal/memory/timezone.go` | EffectiveLocation helper + system_prefs getter/setter (single source of truth for runtime TZ) |
...
For encrypted MCP credentials, generate a 64-hex-char (32-byte) key and put it in `.env` next to `docker-compose.yml` (gitignored): `echo "CURLYCATCLAW_MASTER_KEY=$(openssl rand -hex 32)" >> .env`. Docker Compose reads `.env` automatically and injects the key into the container via `${CURLYCATCLAW_MASTER_KEY:-}` substitution. Without the key, `set_extension_env`/`unset_extension_env` are not registered and any existing `credentials.enc` is unreadable. Both the main process and the MCP subprocess emit a startup `WARN` when `credentials.enc` is on disk but the key is missing, so drift shows up in `docker compose logs curlycatclaw`.
...
Available skills: `/office-hours`, `/plan-ceo-review`, `/plan-eng-review`, `/plan-design-review`, `/design-consultation`, `/design-shotgun`, `/review`, `/ship`, `/land-and-deploy`, `/canary`, `/benchmark`, `/browse`, `/open-gstack-browser`, `/qa`, `/qa-only`, `/design-review`, `/setup-browser-cookies`, `/setup-deploy`, `/retro`, `/investigate`, `/document-release`, `/codex`, `/cso`, `/autoplan`, `/careful`, `/freeze`, `/guard`, `/unfreeze`, `/gstack-upgrade`.
