...
- MCP subprocess debug log: `/data/mcp-debug.log` captures extension loading and hot-reload events. MCP subprocess stderr goes to Claude CLI (invisible in `docker compose logs`), so this file is the only way to debug MCP issues in CLI mode.
