...
- gocron scheduler timezone: `ReminderActor.Run` must create the scheduler via `newCronScheduler(ra.loc)` in `skills/remind.go`, which passes `gocron.WithLocation(ra.loc)` so cron expressions evaluate in the user's configured timezone, not the container's `time.Local` (usually UTC). A bare `gocron.NewScheduler()` makes `0 6 * * *` fire at 06:00 UTC = 23:00 PDT the previous day for a PDT-configured user — 7 hours early. One-time reminders are unaffected (they carry their own zone via absolute `time.Time`). Regression guard: `skills/remind_test.go:TestNewCronScheduler_FiresInConfiguredLocation` schedules the same cron in two zones and asserts the next-runs produce distinct UTC instants, which catches the regression regardless of host `time.Local`.
