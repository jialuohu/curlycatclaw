...
func (a *Actor) buildSystemPrompt(userID, chatID int64, chatType, currentMsg string) string {
		...
		if len(tools) > 0 {
			...
			if ghTools, ok := serverTools["github"]; ok {
				...
				if hasCreateIssue {
					...
					sb.WriteString("NEVER skip the preview step. The user MUST see the draft before submission.\n")
					sb.WriteString("If the tool returns a draft preview, show it to the user and wait for approval.\n")
	...
	sb.WriteString("   - Dockerfile (no other detection files above) → container-based MCP server. Steps:\n")
	sb.WriteString("     a) Check README.md for the exposed port and endpoint path (usually /mcp or /sse)\n")
	sb.WriteString("     b) Build and run: docker build -t <name>-mcp . && docker run -d -p 8080:PORT --name <name>-mcp <name>-mcp\n")
	sb.WriteString("     c) Do NOT use add_extension. Instead, tell the user to add this to config.toml:\n")
	sb.WriteString("        [[mcp.servers]]\n")
	sb.WriteString("        name      = \"<name>\"\n")
	sb.WriteString("        transport = \"http\"\n")
	sb.WriteString("        url       = \"http://localhost:PORT/mcp\"\n")
	sb.WriteString("     d) If the server needs API keys, add [mcp.servers.headers] with the required keys\n")
	sb.WriteString("     e) Restart curlycatclaw to pick up the new server\n")
