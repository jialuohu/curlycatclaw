...
// Load reads a personality markdown file and returns a Persona.
...
func Load(filePath string) (*Persona, error) {
	...
	if len(data) > maxFileSize {
		return nil, fmt.Errorf("personality: file %q is %d bytes, max allowed is %d", filePath, len(data), maxFileSize)
	}
	if !utf8.Valid(data) {
